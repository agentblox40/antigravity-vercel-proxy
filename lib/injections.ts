...
// ⚡ Bolt Optimization: Pre-compile and hoist in-chat control command RegExp patterns to module scope
const VIEW_SETTINGS_TAG_REGEX = /^<(?:MYSETTINGS|SETTINGS|MY_SETTINGS|MY_CONFIG)>\s*$/i;
const VIEW_SETTINGS_SLASH_REGEX = /^\/settings\s*$/i;
const MASTER_TOGGLE_REGEX = /^<INJECTIONS\s*:\s*(ON|OFF|PAUSE|RESUME|ENABLE|DISABLE)>\s*$/i;
const ENABLE_COMMAND_REGEX = /^<(?:ENABLE|ENABLED|ACTIVATE)\s*:\s*([^>]+)>\s*$/i;
const DISABLE_COMMAND_REGEX = /^<(?:DISABLE|DISABLED|DEACTIVATE)\s*:\s*([^>]+)>\s*$/i;
...
export function detectInChatCommand(rawText: string): InChatCommand | null {
  ...
  if (VIEW_SETTINGS_TAG_REGEX.test(trimmed) || VIEW_SETTINGS_SLASH_REGEX.test(trimmed)) {
  ...
  const enableMatch = ENABLE_COMMAND_REGEX.exec(trimmed);
  if (enableMatch) {
  ...
  const disableMatch = DISABLE_COMMAND_REGEX.exec(trimmed);
  if (disableMatch) {
