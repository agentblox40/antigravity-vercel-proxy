...
export function extractCharacterName(systemPrompt: string, messages?: any[]): string {
  ...
  if (cleanPrompt) {
    // Optimized regex patterns to avoid catastrophic backtracking on long system prompts
    const strictPatterns = [
      ...
      // Refactored pattern to eliminate catastrophic backtracking caused by unanchored `[A-Z][a-zA-Z0-9_\-\s']{1,25}`
