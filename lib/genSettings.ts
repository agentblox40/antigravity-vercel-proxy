...
export async function saveGlobalGenSettings(settings: GenerationSettings): Promise<void> {
  memoryGenSettings = { ...DEFAULT_GENERATION_SETTINGS, ...settings };
...
export function parseGenerationSettingsString(input: string): { settings: Partial<GenerationSettings>; parsedSummary: string[] } {
  ...
  for (const pair of pairs) {
    ...
    if (eqIdx === -1) continue;
    const rawKey = pair.slice(0, eqIdx).trim().toLowerCase().replace(/[-_]/g, '');
    const rawVal = pair.slice(eqIdx + 1).trim().toLowerCase();
    ...
    if (rawKey === 'minp') {
    ...
    } else if (rawKey === 'reppenalty' || rawKey === 'repetitionpenalty' || rawKey === 'rep' || rawKey === 'repetition') {
      const val = parseFloat(rawVal);
      if (!isNaN(val) && val >= 1 && val <= 3) {
        result.repetition_penalty = val;
        parsedSummary.push(`repetition_penalty = ${val}`);
      }
    } else if (rawKey === 'freqpenalty' || rawKey === 'frequencypenalty' || rawKey === 'freq' || rawKey === 'frequency') {
      const val = parseFloat(rawVal);
      if (!isNaN(val) && val >= -2 && val <= 2) {
        result.frequency_penalty = val;
        parsedSummary.push(`frequency_penalty = ${val}`);
      }
    } else if (rawKey === 'prespenalty' || rawKey === 'presencepenalty' || rawKey === 'pres' || rawKey === 'presence') {
      const val = parseFloat(rawVal);
      if (!isNaN(val) && val >= -2 && val <= 2) {
      ...
      if (rawVal.endsWith('k')) {
        num = parseInt(rawVal.replace('k', ''), 10) * 1024;
      }
      if (!isNaN(num) && num > 0) {
        result.max_tokens = num;
        parsedSummary.push(`max_tokens = ${num.toLocaleString()}`);
      }
    } else if (rawKey === 'thinking' || rawKey === 'thinkingbudget' || rawKey === 'budget') {
      ...
      if (['off', 'low', 'medium', 'high', 'max'].includes(rawVal)) {
        result.reasoning_effort = rawVal as any;
        const budgetMap: Record<string, number> = { off: 0, low: 2048, medium: 8192, high: 24576, max: 65536 };
        result.thinking_budget = budgetMap[rawVal];
        parsedSummary.push(`reasoning_effort = ${rawVal} (${budgetMap[rawVal].toLocaleString()} tok)`);
      }
    }
  }
...
export function generateGenSettingsMenu(currentSettings: GenerationSettings, notice?: string): string {
  const s = { ...DEFAULT_GENERATION_SETTINGS, ...currentSettings };
